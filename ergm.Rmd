...
```{r, illustration_markov, echo=FALSE, fig.width=3, fig.height=3, out.width=NULL, no.mar=TRUE}
g <- igraph::graph.formula(A -- B, B -- C)
l <- matrix(c(1,2, 2,1, 3,2), 3, 2, byrow=TRUE)
igraph::plot.igraph(g, layout=l, vertex.color="white", vertex.size=40)
```

> TODO describe on figure

Markov dependence assumes that dyads are independent *unless*
share an actor on one of the endpoints.
...
## Other forms of dependence           {#ass_pci}

Partial conditional independence pattison & robins.

```{r, illustration_pcd, echo=FALSE, fig.width=3, fig.height=3, out.width=NULL, no.mar=TRUE}
g <- igraph::graph.formula(A -- B, B -- C, C -- D, D -- A)
...
> TODO summarize following subsections
...
Explicit modeling of degree distributions.

> TODO Modelowanie sieci między sędziami.

```{r, judges_data, eval=FALSE}
...
f$m1 <- update(f$m0, . ~ kstar(1:3))
f$m2 <- update(f$m0, . ~ kstar(2:3) + triangle)
models.mple <- lapply(f, function(x) ergm(x, estimate="MPLE"))
models.bayes <- lapply(f, function(x) bergm(x))
for(m in models.bayes) bergm.output(m)
m1 <- ergm(f$m1, control=control.ergm(MCMC.burnin=50000, MCMC.samplesize=2000, MCMC.interval=3000))
mcmc.diagnostics(m1)
m1.gof <- gof(m1)
layout(matrix(1:4, 2, 2, byrow=TRUE))
plot(m1.gof)
...
# m2
m2 <- ergm(f$m2, constraints=~edges, verbose=TRUE,
           control=control.ergm(MCMC.burnin=50000, MCMC.samplesize=1000, MCMC.interval=10000))
mcmc.diagnostics(m2)
m2.gof <- gof(m2)
layout(matrix(1:4, 2, 2, byrow=TRUE))
plot(m2.gof)
s <- simulate(m2, nsim=100)
...
```{r, ibe_markov, eval=FALSE}
f <- list(m0 = formula( ibe ~ edges ))
f$m1 <- update(f$m0, . ~ . + ostar(2:3) + istar(2:3))
models <- lapply(f, function(m) ergm(m))
lapply(models, summary)
m1.gf <- gof(models$m1)
layout(matrix(1:4, 2, 2, byrow=TRUE))
plot(m1.gf)
m2 <- ergm(ibe ~ edges + 
           gwidegree(0.1, fixed=TRUE) + gwodegree(0.1, fixed=TRUE) +
           nodematch("female") )
mcmc.diagnostics(m2)
gof2 <- gof(m2)
layout(matrix(1:4, 2, 2, byrow=TRUE))
