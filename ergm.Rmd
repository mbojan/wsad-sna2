...
The workhorse for estimating ERG models is the `ergm` function from the `ergm` package. We will described in detail later. Function `ergm` uses a formula interface to specify the network statistics, or `terms`. See `?ergm.terms` for the whole list. If you simply want to calculate the values of network statistics in a given network you can use function `summary`.

For example taking `faux.mesa.high` network, below we calculate number of edges, number of triangles, and number of edges between students of the same sex you can:
