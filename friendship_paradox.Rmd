...
Based on the examples in the previous section we see that average number of friends of individuals (average degree) is never greater, but usually less, then average number of friends of friends. 
The reason for this seemingly paradoxical fact is that people with many friends, like A in `star` network, or C in `stemleaf` network above, have much much more. Take A in the `star` network:

- A's contribution to average degree is 4.
- A's contribution to the average degree of friends is $4 \times 4$ because four people mention A as a friend who has 4 connections himself.
