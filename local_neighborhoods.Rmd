...
There are multiple approaches to studying social networks. In this chapter we will focus on basic properties of ego-networks extensively used in many social studies. Ego-network data are a bit more easy to collect as compared to complete network data and can be used to make inferences for large populations [@everett_borgatti2005]. Ego-networks have been used in studies of: social support, knowledge sharing, or disease spread. Examples of ego-network measures can now be encountered on social networking websites, e.g. LinkedIn.

Even if one has complete network data it may be still of interest to "disassemble" the complete network into a set of ego-networks, or neighborhoods [@everett_borgatti2005].
 Such an exercise might facilitate understanding the variation across actors embedded in their "local" environments.
 
Let us start with defining two basic concepts:

a) *Ego* is an individual node identified in the network structure. Depending on research goals, egos can be persons, groups, societies, organizations, firms, states etc. Ego-network is a "set of alters who have ties to ego, and measurements on the ties among these alters" [@wasserman_faust_1994: 42].
...
```{r neighborhood_defined, echo = FALSE}
...
for (i in seq_along(neighborhoods)) {
  ...
  plot(g, main = titles[i], vertex.label = NA)
}
par(op)
```
...
```{r neighbors_networks, echo=1:2}
g <- graph.famous("Frucht")
g_d <- graph(c(1,2, 1,3, 2,1, 2,4, 3,4, 4,3, 1,5, 5,4, 6,3, 4,6, 5,7))
layout(matrix(1:2, 1, 2))
op <- par( mar=rep(0.5, 4) )
plot(g, vertex.color="lightskyblue", vertex.label.color="black")
plot(g_d, vertex.color="lightskyblue", vertex.label.color="black",
     edge.arrow.size=0.5, edge.curved=0.1)
par(op)
```
...
Plotting neighborhood graphs of vertices `r paste(letters[1:6], collapse=", ")`:

```{r plot_graph_neighborhood}
layout(matrix(1:6, 2, 3, byrow=TRUE))
...
```{r plot_graph_neighborhood_directed}
layout(matrix(1:6, 2, 3, byrow=TRUE))
op <- par(mar=rep(0.1, 4))
for (graph in ego_networks) {
  plot(graph, vertex.color="lightskyblue",
...
Article by Paul Nieuwbeerta and Henk Falp shows how ego characteristics can be used [@nieuwbeerta_flap_2000]
...
Consider the classroom network. We want to asses to what extent children prefer to play with colleagues of the same sex. As we are interested in ego's preference towards others and not the other way round, we will analyze out-neighborhood.
...
hist(rmsd, xlab = "Root mean squared difference between \nsocioeconomic status of ego and its alters", 
     col="grey", border="white",
     main = "")
abline(v = mean(rmsd, na.rm = TRUE), col="red", lwd=2)
```
```{r, include = FALSE, eval=FALSE}
# histogram of fractions with respect to gender
...
data(Wnet)
par(mar = rep(0,4))
plot(Wnet, vertex.size = 20, vertex.label.cex = 0.8, vertex.color="lightskyblue")
...
par(mar=rep(0,4))
plot(g, vertex.size = 40, vertex.color="lightskyblue")
```
