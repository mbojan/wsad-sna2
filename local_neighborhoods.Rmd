...
Consider this simple graph, that represents ego-network of John.
...
Here John serves a broker between Lara and both Peter and Sara, because Sara has no direct connection to them
...
In our example of John's ego-network there are structural holes between Lara and both Peter and Sara.
...
See how efficiency and effective size are distributed in kinship network --> TUTAJ JAKAŚ INNA SIEĆ DO WYBRANIA - COAUTHORSHIP PEWNIE BYŁBY DOBRY, TYLKO JAKAŚ W MIARĘ MAŁĄ SIEĆ TRZEBA ZNALEŹĆ
```{r kable}
data(Wnet)
plot(Wnet)
